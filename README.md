...
COMFYUI_SERVER_ADDRESS=127.0.0.1  # ComfyUI 服務器地址（圖像生成功能）
COMFYUI_SERVER_PORT=8188          # ComfyUI 服務器端口
HOST=0.0.0.0  # 網路存取設定
PORT=5001     # 服務端口
...
2. **語音生成問題**
   - **ComfyUI 連接失敗**：確認 ComfyUI 服務正在運行（預設 http://localhost:8188，用於圖像生成功能）
   - **Index-TTS 語音生成失敗**：Index-TTS 使用獨立的服務器配置（192.168.68.30:8000）
